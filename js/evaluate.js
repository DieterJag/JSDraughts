...
function EvalPosition(alpha, beta) {
    ...
    if ( (count_bk == 0) && (count_wk == 0) ){
        ...
        if ( count_bm == count_wm && count_bm + count_wm <= 12 ){
            ...
            if ( brd_side == COLORS.BLACK && has_man_on_7th_b() == 0 ) { // need change
                for ( i=5; i <= 8;i++) {
                    for ( j=0; j < 4; j++) {
    ...
    for ( i = 1;i <= num_bpieces;i++){
        ...
        if ( color == BLACK )
        if (( brd_pieces[23] != PIECES.wM ) || ( brd_pieces[19] != PIECES.bM ))
            eval += 8;
            if ( color == WHITE )
        if (( brd_pieces[22] != PIECES.bM ) || ( brd_pieces[26] != PIECES.wM ))
            eval -= 8;
                                                                                        }
                                                                                        }
                                                                                    }
